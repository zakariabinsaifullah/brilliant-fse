...
const NativeIconPicker = ({
    ...
    return (
        <>
            ...
            />
            <BaseControl id="brilliant-icon-settings" label={label} __nextHasNoMarginBottom>
                <Dropdown
                    popoverProps={{
                    ...
                    className="brilliant-icon-settings"
                    renderToggle={({ isOpen, onToggle, onClose }) => (
                        <div className="brilliant-icon-settings__dropdown">
                            <Button
                                onClick={onToggle}
                                aria-expanded={isOpen}
                                className="brilliant-icon-settings__dropdown-toggle"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            >
                                {!!(iconName || customSvgCode) ? (
                                    <span className="brilliant-icon-settings__indicator">
                                        <RenderIcon customSvgCode={customSvgCode} iconName={iconName} size={20} />
                                    </span>
                                ) : (
                                    <span className="brilliant-icon-settings__indicator disabled"></span>
                                )}

                                <span className="brilliant-icon-settings__dropdown-label">
                                    {__(iconName || (customSvgCode ? 'Custom SVG' : 'Select Icon', 'brilliant'))}
                                </span>
                            </Button>
                            <Button
                                label={__('Browse library', 'brilliant')}
                                onClick={() => {
                                    onClose();
                                    openModal('library');
                                }}
                                ...
                                size="small"
                                icon={folderOpen}
                                className="brilliant-icon-settings__dropdown-more"
                                __next40pxDefaultSize
                                __nextHasNoMarginBottom
                            />
                        </div>
